...
-   强制/显式类型转换 explicit conversion

    -   (类型名) 待转换数据 // C 风格转换语句

        -   类型名(待转换数据) //C++风格转换语句

        -   自动/隐式类型转换 implicit conversion  
            ...
            -   3 算术运算符求解：7 / 2.718;  
                 7 转换成 double 型，让两边一致，再做除法，得数也是 double 型
...
-   结构体
    ...
    -   创建
        ...
        -   struct 结构体名 变量名; //struct 可省 变量名 . 结构体成员 1= ; 变量名 . 结构体成员 2= ;

            -   struct humans hum1; hum1 . age= 24 ; hum1 . sex= "男" ;

        -   struct 结构体名 变量名= { 结构体成员列表赋值 };
...
-   -   内存大小
    ...
    -   输入数据类型错误时,变量的值可能是随机值

    -   userInput 的功能是过滤非法输入
